...
async function analyzeUrl() {
    ...
    if (!url) {
        showNotification('请输入有效的网址');
        return;
    }

    if (!url.startsWith('http')) {
        showNotification('网址必须以 http 或 https 开头');
        return;
    }

    const statusDiv = document.getElementById('crawler-status');
    statusDiv.textContent = '正在分析页面...';
    statusDiv.style.color = '#666';

    document.getElementById('crawler-preview').style.display = 'none';
    document.getElementById('crawler-actions').style.display = 'none';

    try {
        statusDiv.textContent = '正在调用 Python 爬虫分析链接... (这可能需要几秒钟)';

        // 调用 Python 爬虫
        const chapters = await ipcRenderer.invoke('run-python-crawler', url);

        if (!chapters || chapters.length === 0) {
            statusDiv.textContent = '未找到章节链接，请检查网址是否正确';
            statusDiv.style.color = 'red';
            return;
        }

        // 提取书名 (Python 脚本目前只返回章节，我们可以简单从 URL 或第一个章节推断，或者让 Python 返回更多信息)
        // 这里简单处理，如果 Python 没返回书名，就用默认的
        crawlerBookTitle = 'Unknown_Novel';
        // 尝试从页面标题获取书名 (虽然我们现在是用 Python 抓取的，但为了书名可能还需要 fetch 一次？)
        // 或者我们修改 Python 脚本返回结构。
        // 暂时先用一个占位符，或者再次 fetch 首页获取标题
        try {
            const buffer = await ipcRenderer.invoke('fetch-url', url);
            const decoder = new TextDecoder('utf-8');
            const html = decoder.decode(buffer);
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            const h1 = doc.querySelector('h1');
            if (h1) crawlerBookTitle = h1.textContent.trim();
        } catch (e) {
            console.log('获取书名失败', e);
        }

        console.log(`Python 爬虫找到 ${chapters.length} 章`);
...
/* 
// 旧的 JS 爬虫逻辑 (已废弃，改用 Python)
async function analyzeUrlOld(url) {
    // ... (原有代码)
}
*/

function renderChapterList(chapters) {
    const listDiv = document.getElementById('crawler-chapter-list');
    listDiv.innerHTML = '';

    const statusDiv = document.getElementById('crawler-status');
    statusDiv.textContent = `找到 ${chapters.length} 个章节 (书名: ${crawlerBookTitle})`;
    statusDiv.style.color = 'green';

    // ... (后续渲染逻辑保持不变，但需要注意 chapters 结构)
    // Python 返回的是 [{title, url}, ...]

    crawlerChapters = chapters; // 更新全局变量
    ...
    document.getElementById('crawler-preview').style.display = 'block';
    document.getElementById('crawler-actions').style.display = 'block';
}
...
function parseChineseNumber(str) {
    const cn = { '零': 0, '一': 1, '二': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9, '十': 10, '百': 100, '千': 1000, '万': 10000 };
    // 简单处理常见情况
    ...
    return result === 0 ? 0 : result; // 避免完全解析失败
}

async function startCrawling() {
    if (isCrawling) return;
    if (crawlerChapters.length === 0) return;

    isCrawling = true;
    const progressContainer = document.getElementById('crawler-progress-container');
    const progressBar = document.getElementById('crawler-progress-bar');
    const progressText = document.getElementById('crawler-progress-text');
    const logDiv = document.getElementById('crawler-log');

    progressContainer.style.display = 'block';
    document.getElementById('crawler-actions').style.display = 'none'; // 隐藏开始按钮
    ...
    for (let i = 0; i < crawlerChapters.length; i += batchSize) {
        const batch = crawlerChapters.slice(i, i + batchSize);
        const promises = batch.map(async (chapter) => {
            // 简单的重试机制
            let retries = 3;
            while (retries > 0) {
                try {
                    logDiv.textContent = `正在获取: ${chapter.title}`;

                    const buffer = await ipcRenderer.invoke('fetch-url', chapter.url);
                    if (buffer.error) throw new Error(buffer.error);
                    ...
                    // 优先查找常见ID
                    const commonIds = ['content', 'chapter-content', 'text', 'txt', 'book_text', 'nr1'];
                    ...
                    if (!contentDiv) {
                        ...
                        for (const div of divs) {
                            // 排除包含大量链接的 div (可能是导航或推荐)
                            if (div.querySelectorAll('a').length > 5) continue;
                    ...
                    if (contentDiv) {
                        // 清理内容
                        // 移除 script, style
                        const scripts = contentDiv.querySelectorAll('script, style, div[class*="ad"], .ads');
                        ...
                        text = text.replace(/\n\s*\n/g, '\n\n').trim();
                        // 移除可能的广告文本
                        text = text.replace(/.*(阅读|下载|APP|网址).*/g, '');
        ...
        successCount += results.length; // 简化统计

        // 更新进度
        const progress = Math.min(((i + batch.length) / crawlerChapters.length) * 100, 100);
        progressBar.style.width = `${progress}%`;
        progressText.textContent = `${Math.min(i + batch.length, crawlerChapters.length)}/${crawlerChapters.length}`;

        // 随机延时，避免请求过快被封 (不择手段也要稍微注意一点点，防止直接断连)
        await new Promise(r => setTimeout(r, Math.random() * 500 + 200));
    }

    logDiv.textContent = '下载完成，正在保存...';

    // 保存文件
    try {
        // 使用提取的书名作为文件名
        let fileName = crawlerBookTitle || 'novel';
        // 移除非法字符
        fileName = fileName.replace(/[\\/:*?"<>|]/g, '_');

        const result = await ipcRenderer.invoke('save-text-file', {
            content: fullContent,
            defaultName: `${fileName}.txt`
        });

        if (result.success) {
            showNotification(`保存成功: ${result.filePath}`);
            toggleCrawler(); // 关闭面板

            // 询问是否立即打开
            if (confirm('下载完成，是否立即打开阅读？')) {
                // 模拟文件选择
                // 这里比较复杂，因为我们需要 File 对象或者路径
                // 我们可以直接调用 loadAndRenderBook 如果我们有路径
                // 但 loadAndRenderBook 需要 File 对象或者路径逻辑
                // 我们可以简单地重置界面
                window.location.reload();
            }
        } else if (result.canceled) {
            showNotification('取消保存');
        } else {
            showNotification('保存失败: ' + result.error);
        }
    } catch (error) {
        showNotification('保存出错: ' + error.message);
    }
